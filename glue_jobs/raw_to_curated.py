...
deduped_df = (
    valid_status_df.withColumn("_rank", F.rank().over(window_spec))
    .filter(col("_rank") == 1)
    .drop("_rank")
)
...
deduped_df = (
    deduped_df.withColumn("order_date", to_date(col("created_at")))
    .withColumn("hour_of_day", hour(col("created_at")))
    .withColumn("total_amount", col("quantity") * col("unit_price"))
    .withColumn(
        "is_weekend", (F.dayofweek(col("created_at")).isin([1, 7])).cast("boolean")
    )
)
...
try:
    ...
    deduped_with_type = (
        ...
        .withColumn(
            "record_type",
            when(col("existing_order_id").isNotNull(), lit("UPDATE")).otherwise(
                lit("INSERT")
            ),
        )
...
if missing_count > 0:
    ...
    q = q.withColumn(
        "order_date",
        when(col("updated_at").isNotNull(), to_date(col("updated_at"))).otherwise(
...
if invalid_values_count > 0:
    q = invalid_values_df.withColumn(
        "rejection_reason", lit("invalid_quantity_or_price")
    )
    q = q.withColumn(
        "order_date",
        when(col("updated_at").isNotNull(), to_date(col("updated_at"))).otherwise(
...
deduped_with_type.write.mode("append").partitionBy("order_date").parquet(CURATED_PATH)
