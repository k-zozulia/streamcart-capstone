"""
Glue ETL job for StreamCart capstone.
...
args = getResolvedOptions(sys.argv, ["JOB_NAME", "S3_INPUT_PATH", "PIPELINE_RUN_ID"])
...
# Dynamic partition overwrite (Only rewrites partitions (order_date) that appear in the current batch.)
...
raw_df = spark.read.schema(raw_schema).json(S3_INPUT_PATH)
input_count = raw_df.count()
print(f"INPUT rows: {input_count}")

# Step 2 — Filter missing order_id → quarantine
# In practice order_id is always set (Postgres SERIAL PK) but we keep
# this check for robustness when data comes from other sources.
...
valid_values_df   = valid_df.filter((col("quantity") > 0) & (col("unit_price") >= 0))
invalid_values_df = valid_df.filter((col("quantity") <= 0) | (col("unit_price") < 0))
invalid_values_count = invalid_values_df.count()
print(f"Invalid quantity/price: {invalid_values_count}")

# Step 4 — Normalize and validate status
...
# Step 5 — Deduplicate within current batch
# Keeps the latest updated_at per order_id within this CDC batch.
# Note: business-key duplicates (same customer+product+qty+price+status but
# different order_id) are intentionally kept here — dbt handles those via
# int_orders_quality_flags.is_duplicate.
...
deduped_df = (
    ...
    .withColumn("is_weekend",   (F.dayofweek(col("created_at")).isin([1, 7])).cast("boolean"))
)

# Step 7 — Detect INSERT vs UPDATE
# Compare incoming order_ids with existing curated data.
# We only TAG records as INSERT or UPDATE here.
try:
    ...
    print(f"No existing curated data (first run): {e}")
    deduped_with_type = deduped_df.withColumn("record_type", lit("INSERT"))

# Add pipeline_run_id
deduped_with_type = deduped_with_type.withColumn(
...
if invalid_values_count > 0:
    ...
    q = q.withColumn("pipeline_run_id", lit(PIPELINE_RUN_ID))
...
if quarantine_parts:
    ...
    # Quarantine also uses dynamic overwrite
    quarantine_df.write.mode("overwrite").partitionBy("order_date").parquet(QUARANTINE_PATH)
else:
    print("QUARANTINE rows: 0")

# ── Step 9 — Write curated Parquet (dynamic partition overwrite) ──────────
# mode=overwrite + partitionOverwriteMode=dynamic rewrites ONLY the
# order_date partitions present in this batch. All other partitions
# (dates not touched by this run) remain unchanged in S3.
